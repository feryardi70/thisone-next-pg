...
export default function ViewArrival() {
  ...
  useEffect(() => {
    const fetchDepartures = async () => {
      try {
        ...
        const response = await axiosInstance.get("/arrival/viewarrival", { params: { arrivedate: formattedDay } });
  ...
  return (
    <div className="m-0 min-h-screen text-white text-2xl bg-green-700">
      <table className="w-full">
        <thead className="text-3xl bg-indigo-900">
          <tr>
            <th className="border-b-2 border-b-red-700 p-2 text-center w-[25%]">Flight</th>
            <th className="border-b-2 border-b-red-700 p-2 text-center">Origin</th>
            <th className="border-b-2 border-b-red-700 p-2 text-center">Time</th>
            <th className="border-b-2 border-b-red-700 p-2 text-center">Baggage Claim</th>
          ...
          {arrivals.length > 0 ? (
            arrivals.map((arrive, index) => (
              <tr key={index}>
                <td className="border-b border-white p-2 w-[25%]">
                  <div className="flex flex-row items-center justify-start">
                    ...
                    <div className="ml-5">
                      ...
                      <span className="ml-1">{arrive.flightnumber}</span>
                    </div>
                  </div>
                </td>
                ...
                <td className="border-b border-white p-2 text-center">{arrive.arrivetime}</td>
                <td className="border-b border-white p-2 text-center">{arrive.baggage}</td>
                <td className="border-b border-white p-2 text-center" style={{ textTransform: "uppercase" }}>
                  {arrive.remark}
                </td>
              </tr>
      ...
      <div className="flex justify-center px-7 mt-14">
        <div className="w-full bg-teal-900 flex flex-col justify-center items-center py-3">
          <div className="text-6xl tracking-widest">ARRIVAL</div>
